...
const PRIMITIVE_TYPES: &[(&str, &str)] = &[
    ("i32",  "Signed 32-bit integer. Default type for integer literals."),
    ("i64",  "Signed 64-bit integer."),
    ("f32",  "32-bit single-precision float."),
    ("f64",  "64-bit double-precision float. Default type for float literals."),
    ("bool", "Boolean type. Values: true or false."),
    ("str",  "String type — pointer to a null-terminated C string."),
    ("u32",  "Unsigned 32-bit integer. Uses unsigned semantics for division and comparison."),
    ("void", "Unit/void return type. Used when a function returns nothing."),
];
...
struct StdlibInfo {
    uri:     String,
    entries: Vec<(String, usize, usize)>,  // (name, 0-based line, 0-based col)
}
...
fn write_stdlib() -> Option<StdlibInfo> {
    ...
    let header = "// Vyrn Standard Library — Built-in Functions & Primitive Types\n\
                  //\n\
                  // Auto-generated by vyrn-lsp. Do not edit.\n\
                  \n";
    let mut content = header.to_string();
    let mut entries = Vec::new();
    // header = lines 0-3; each entry: comment line, decl line, blank line (3 lines each)
    let mut cur = 4usize;

    for b in BUILTINS {
        entries.push((b.name.to_string(), cur + 1, 3)); // col 3 = after "fn "
        content.push_str(&format!("// {}\nfn {}(...) {{}}\n\n", b.doc, b.name));
        cur += 3;
    }

    // Separator before types section (3 lines: blank, header comment, blank)
    content.push_str("\n// ── Primitive Types ─────────────────────────────────────────────────────\n\n");
    cur += 3;

    for (name, doc) in PRIMITIVE_TYPES {
        entries.push((name.to_string(), cur + 1, 5)); // col 5 = after "type "
        content.push_str(&format!("// {}\ntype {}\n\n", doc, name));
        cur += 3;
    }
        ...
        match method.as_str() {
            ...
            "textDocument/definition" => {
                ...
                let result: Value = (|| {
                    ...
                    // Built-ins / primitive types → jump to generated stdlib stub
                    if let Some(ref sl) = stdlib {
                        if let Some(&(_, decl_line, decl_col)) = sl.entries.iter().find(|(n, _, _)| n == &word) {
                            return Some(json!({
                                "uri": sl.uri,
                                "range": {
                                    "start": { "line": decl_line, "character": decl_col },
                                    "end":   { "line": decl_line, "character": decl_col + word.len() }
                                }
                            }));
                        }
                    }
